...
float hor_pixel_angle = 0.18125,ver_pixel_angle = 0.18750,x_speed = 0.3,dist_1 = 0,dist_2 = 0,z_angle = 0.2,turn_angle = 32,data_count = 500;//45/240;//58/320;
float move_distance = 75, average_dist = 0;
double pi = 3.1415926535897931,one_degree = pi/180,old_yaw = 0,sec_initial,sec_final;
double old_x = 0,old_y = 0,new_x = 0,new_y=0;
string directory = "/home/hisham/iitm_ra/drdo_data2/",folder = "set_right" ,name = "/kinect_depth_" ,ext =".bmp";
...
void get_featureIn(const sensor_msgs::ImageConstPtr& msg, float eff_status, int pos){
    cv::Mat a = normalFeature(msg).t();
    cv::Mat b = (Mat_<float>(1,1) << eff_status);

    if(pos == 0){
        folder_count_F++;
        entityMatF.push_back(a);
        effectMatF.push_back(b);
...
void forwardCallback(const sensor_msgs::ImageConstPtr& msg){
    if(get_client.call(getmodelstate)){
        if(checkOrientation()){
            ...
            if((sec_final - sec_initial) >= 3){
                ROS_INFO("capturing second image");
                check_point = false;
                cmdvel_.linear.x = 0;
                cmd_pub_.publish(cmdvel_);
                ROS_INFO("distance = %f ", dist);
                float st = (dist >= move_distance - 0.05 && dist <= move_distance + 0.05)?1:0;
                ROS_INFO("forward status = %f", st);
                get_featureIn(image_1, st, 0);
                ROSLoop = false;
            }
        }else{
            ROSLoop = false;
    ...
    if(get_client.call(getmodelstate)){
        ...
        if(checkRobotError(roll,pitch)){
            ...
            if((sec_final - sec_initial) >= 2){
                ...
                if(old_yaw > yaw){
                    //this means there is  an angle turn over. 359 -> 360 -> 1 -> 2
                    rotation_angle = (pi - old_yaw) + (yaw + pi);
                }else{
                    rotation_angle = yaw - old_yaw;
                }
                ROS_INFO("left = %f, %f", rotation_angle,(turn_angle)*one_degree);
                float st = (rotation_angle >= (turn_angle-4)*one_degree && rotation_angle <= (turn_angle+4)*one_degree)?1:0;
                ROS_INFO("left status = %f", st);
                get_featureIn(image_1, st, 1);
                ROSLoop = false;
            }
        }else{
            ROSLoop = false;
    ...
    if(get_client.call(getmodelstate)){
        ...
        if(checkRobotError(roll,pitch)){
            ...
            if((sec_final - sec_initial) >= 2){
                ...
                double rotation_angle = 0;
                if(old_yaw < yaw){
                    //this means there is  an angle turn over. 2 -> 1 -> 360 -> 359
                    rotation_angle = (old_yaw + pi) + (pi - yaw);
                }else{
                    rotation_angle = old_yaw - yaw;
                }
                ROS_INFO("right = %f, %f", rotation_angle,(turn_angle)*one_degree);
                float st = (rotation_angle >= (turn_angle-4)*one_degree && rotation_angle <= (turn_angle+4)*one_degree)?1:0;
                ROS_INFO("right status = %f", st);
                get_featureIn(image_1, st, 2);
                ROSLoop = false;
            }
        }else{
            ROSLoop = false;
